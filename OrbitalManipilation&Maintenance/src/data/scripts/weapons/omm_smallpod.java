...
public class omm_smallpod implements EveryFrameWeaponEffectPlugin {

    private SpriteAPI sprite;
    private Vector2f size;
    private boolean isWeaponSwappedsmall = false;
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (this.SHIP.getOriginalOwner() == 0 || this.SHIP.getOriginalOwner() == 1) { //check for refit screen
            ...
            for (FighterWingAPI fighterWingAPI : dronewings) {
                if (!fighterWingAPI.getWingId().equals("omm_smallpod_wing")) {   //name is the built-in drone wing
                    continue;
                }
...
//                }
                            if (weaponAPI.getSlot().getId().equals("smlhybridslot")) {                //slot has same name as on drone !important!
//                    weaponAPI.getSprite().setColor(new Color(255, 255, 255, 0));
                            }
                            for (int i = 0; i < weapons.size(); i++) {
                                if (weaponAPI.getSlot().getId().equals("smlhybridslot")) {
        ...
        if (!isWeaponSwappedsmall) {
            ...
            if (this.SHIP != null) {
                ...
                for (FighterWingAPI fighterWingAPI : list1) {
                    if (!fighterWingAPI.getWingId().equals("omm_smallpod_wing")) {
                        continue;
                    }

                    {
                        ...
                        mutableShipStatsAPI.getVariant().clearSlot("smlhybridslot");
                        if (this.SHIP.getVariant().getWeaponSpec("smlhybridslot") != null) {
                            mutableShipStatsAPI.getVariant().addWeapon("smlhybridslot", this.SHIP.getVariant().getWeaponId("smlhybridslot"));
                            mutableShipStatsAPI.getVariant().getWeaponSpec("smlhybridslot").addTag("FIRE_WHEN_INEFFICIENT");
                            fighterWingAPI.orderReturn(this.FIGHTER);
                            FIGHTER.getVariant().assignUnassignedWeapons();
                            this.isWeaponSwappedsmall = true;
