    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (this.SHIP.getOriginalOwner() == 0 || this.SHIP.getOriginalOwner() == 1) { //check for refit screen
            ...
            for (FighterWingAPI fighterWingAPI : dronewings) {
                if (!fighterWingAPI.getWingId().equals("omm_weaponpod_wing")) {   //name is the built-in drone wing
                    continue;
                }
                ...
                for (WeaponAPI dronewep : droneweps) {
                    if (dronewep.getSlot().getId().equals("droneslot")) {
...
//                }
                            if (weaponAPI.getSlot().getId().equals("droneslot")) {                //slot has same name as on drone !important!
//                    weaponAPI.getSprite().setColor(new Color(255, 255, 255, 0));
                            }
