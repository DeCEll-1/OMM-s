...
public class limb_move implements EveryFrameWeaponEffectPlugin 
{
  ...
  private final float FREQ = 0.02F;
    ...
    if (this.time >= 0.05F) {
      ...
      if (thrust > 0.0F) {
        ...
        if (!this.turn && this.FRAMES != 0.0F) {
          ...
          if (this.FRAMES == 0.0F) {
            rotate(weapon, combinedAngle, combinedThrust, 0.006F);
          } else {
            thrust(weapon, combinedAngle, combinedThrust, 0.006F);
          } 
        } 
      } else if (this.FRAMES == 0.0F) {
        rotate(weapon, this.NEUTRAL_ANGLE, 0.0F, 0.006F);
      } else {
        thrust(weapon, this.NEUTRAL_ANGLE, 0.0F, 0.006F);
      } 
    } 
  }
